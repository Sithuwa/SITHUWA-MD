...
var acr = /** @class */ (function () {
    ...
    acr.prototype.sign = function (string, access_secret) {
        return crypto.createHmac("sha1", access_secret).update(Buffer.from(string, "utf-8")).digest().toString("base64");
    };
    //  Generates form data from an object
    acr.prototype.generateFormData = function (object) {
        var form = new FormData();
        Object.keys(object).forEach(function (key) { form.append(key, object[key]); });
        return form;
    };
    /**
    ...
    acr.prototype.identify = function (audio_sample) {
        ...
        return fetch("https://" + this.host + "/" + this.endpoint, {  method: "POST", body: this.generateFormData(formData) }).then(function (response) { return response.json(); });
    };
    return acr;
}());
module.exports = acr;
